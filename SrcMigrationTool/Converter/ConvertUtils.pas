...
type
  TUsesIndex = record
    IntfUsesSIdx,   // interface uses 절 시작
    IntfUsesEIdx,
    ImplIdx,        // implimentation 시작
    ImplUsesSIdx,   // implimentation uses 절 시작
    ImplUsesEIdx: Integer;
  end;

// 들여쓰기 추가
procedure WriteIndent(var AList: TStringList; ACount: Integer);
// DFM 문자열의 StringList 중 AParent에 AChild를 부모로 추가
procedure InsertChildCompText(AParent, AChild: TStringList);
procedure RemoveParentName(var ACompStrings: TStringList);
procedure ComponentToStringList(AComponent: TComponent; AStringList: TStringList;
              const ARemoveParentName: Boolean);
function ComponentToText(AComponent: TComponent): string;

// DFM 문자열 중 object ObjName: TDataType 에서 ObjName을 반환
...
procedure SearchUsesIndex(AList: TStringList; var UsesIdx: TUsesIndex);
...
var
  ...
  Text: string;
begin
  UsesIdx.IntfUsesSIdx := -1;
  UsesIdx.IntfUsesEIdx := -1;
  UsesIdx.ImplIdx := -1;
  UsesIdx.ImplUsesSIdx := -1;
  UsesIdx.ImplUsesEIdx := -1;
  ...
  begin
    Text := AList[I];
    if UsesIdx.IntfUsesSIdx = -1 then
    begin
      if Text.Trim.ToLower.StartsWith('uses') then
        UsesIdx.IntfUsesSIdx := I;
    end
    else if UsesIdx.IntfUsesEIdx = -1 then
    begin
      if (not Text.TrimLeft.StartsWith('//')) and (Text.Contains(';')) then
        UsesIdx.IntfUsesEIdx := I;
    end
    else if UsesIdx.ImplIdx = -1 then
    begin
      if Text.Trim.ToLower.StartsWith('implementation') then
        UsesIdx.ImplIdx := I;
    end
    else if UsesIdx.ImplUsesSIdx = -1 then
    begin
      if Text.Trim.ToLower.StartsWith('uses') then
        UsesIdx.ImplUsesSIdx := I;
    end
    else if UsesIdx.ImplUsesEIdx = -1 then
    begin
      if (not Text.TrimLeft.StartsWith('//')) and (Text.IndexOf(';') > 0) then
        UsesIdx.ImplUsesEIdx := I;
    end
