...
type
  ...
  private
    ...
    procedure RunBackup(AData: TDfmFileData);
  public
    constructor Create;
    ...
    procedure Init;
    function RunConvert(AFileData: TDfmFileData;
    ...
    property ConvertInstances: TList<TConverter> read FConverterInstances;
  end;

  TRemoveConverter = class(TConverter)
  protected
...
procedure TConvertManager.RunBackup(AData: TDfmFileData);
var
  BackupPath: string;
begin
...
function TConvertManager.RunConvert(AFileData: TDfmFileData;
      AConverters: TArray<TConverter>): Integer;
var
...
begin
  ...
  try
    ...
    for Converter in AConverters do
...
begin
  CompClassName := GetTargetCompClassName;
...
begin
  Result := Format('%s to %s Convert', [GetTargetCompClassName, GetConvertCompClassName])
end;
...
begin
  ...
  try
    // 컴포넌트 시작 전 내용 복사
    Converted.BeginUpdate;
    for I := 0 to AData.CompStartIndex - 1 do
      Converted.Add(AData.SrcDfm[I]);

    Converted.AddStrings(AData.ConvDfm);
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          TLogger.Error('[%s] Not found event(Intf) ''%s''', [AData.DfmFileData.Filename, CompCodeInfo.BeforeEventName]);
          Continue;
        end;
        ...
        begin
          ...
          if (CompEventStart = 0) or (CompEventEnd = 0) then
          begin
            TLogger.Error('[%s] Not found event(Impl) ''%s''', [AData.DfmFileData.Filename, CompCodeInfo.BeforeEventName]);
            Continue;
          end;
...
function TRemoveConverter.GetConvertCompClassName: string;
begin
  Result := '';
end;

function TRemoveConverter.GetConvertedCompText(ACompText: TStrings;
...
function TRemoveConverter.GetDescription: string;
begin
  Result := GetTargetCompClassName + ' remove';
end;
